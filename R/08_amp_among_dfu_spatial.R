# =============================================================================
# 08_amp_among_dfu_spatial.R
#
# Conditional amputation analysis: amputation rate per 1,000 person-halfyears
# AMONG DFU PATIENTS (Tier 2). Complements scripts 05 (DFU prevalence) and 07
# (amputation incidence in all DM).
#
# Rationale:
#   - Script 05 measures DFU coding intensity -- confounded by care access
#     and provider coding patterns.
#   - Script 07 measures amputation incidence in all diabetics -- a mix of
#     (DFU development) x (DFU -> amputation progression).
#   - Script 08 isolates the LATTER: given a patient already has coded DFU,
#     what's their probability of progressing to amputation in this ZCTA?
#     This is the "treatment effectiveness / access failure" signal.
#
# A ZCTA where DFU patients routinely progress to amputation is a ZCTA
# where the healthcare system is failing them even after disease is
# identified. This is the single cleanest measure of healthcare access
# failure for PCD-style public health targeting.
#
# Numerator: first amputation date falling within each half-year bin,
#   among patients who have tier2 == 1 (coded DFU).
#
# Denominator: enrollment-weighted person-halfyears among tier2 DFU
#   patients (not all DM patients as in 07). Each Tier 2 patient
#   contributes enrollment months / 6 per bin, same as the main pipeline,
#   but the pool is restricted to patients with coded DFU.
#
# Cohort via DFU_COHORT env var (default = continuous).
#
# Assumptions: R/_assumptions.md sections 3.2, 6.1, 8.2.
# =============================================================================

suppressPackageStartupMessages({
    library(data.table)
    library(sf)
    library(spdep)
    library(tigris)
    library(ggplot2)
    library(fs)
    library(viridis)
})

options(tigris_use_cache = TRUE)
sf_use_s2(FALSE)

# ---- Paths ------------------------------------------------------------------
DROPBOX_DIR <- Sys.getenv(
    "DFU_DROPBOX_DIR",
    "/Users/williamwatson/Library/CloudStorage/Dropbox/Dissertation/Aim 3 - DFU"
)
ANALYTIC <- file.path(DROPBOX_DIR, "analytic")
OUT_DIR  <- file.path(DROPBOX_DIR, "outputs", "static_spatial")
dir_create(OUT_DIR)

COHORT <- Sys.getenv("DFU_COHORT", "continuous")
message("Using cohort: ", COHORT)

# ---- Parameters -------------------------------------------------------------
STUDY_YEARS <- 2017:2022
KNN_K       <- 8L
NPERM       <- 999L
FDR_ALPHA   <- 0.05
DEN_THRESH  <- 5L    # looser than main pipeline -- DFU-only denominators are
                      # inherently smaller; 5 person-halfyears of DFU exposure
                      # is about 1 distinct DFU patient followed all 6 years
BIN_MONTH_FLOOR <- 3L

BIN_COLS <- c("m_h1_2017", "m_h2_2017",
              "m_h1_2018", "m_h2_2018",
              "m_h1_2019", "m_h2_2019",
              "m_h1_2020", "m_h2_2020",
              "m_h1_2021", "m_h2_2021",
              "m_h1_2022", "m_h2_2022")

HALF_BIN_IDS <- c("2017-H1", "2017-H2", "2018-H1", "2018-H2",
                  "2019-H1", "2019-H2", "2020-H1", "2020-H2",
                  "2021-H1", "2021-H2", "2022-H1", "2022-H2")

# ---- Load tiered patient data and cohort enrollment -------------------------
pt <- readRDS(file.path(ANALYTIC, "02_tiered.rds"))
pt <- pt[dm_primary == 1L & tier2 == 1L & !is.na(zcta)]
pt[, apcd_unique_id := fifelse(
    nchar(study_id) >= 2,
    substr(study_id, 1, nchar(study_id) - 1L),
    NA_character_
)]
pt[, first_amp_date := as.Date(first_amp_date)]
pt[, first_dfu_date := as.Date(first_dfu_date)]

ENROLL_DIR <- Sys.getenv(
    "DFU_ENROLL_DIR",
    "/Users/williamwatson/Library/CloudStorage/Dropbox/APCD/Analytical Datasets"
)
cohort_csv <- switch(COHORT,
    "continuous" = "cohort_continuous.csv",
    "fractional" = "cohort_fractional.csv",
    stop("Unknown cohort: ", COHORT)
)
cohort_dt <- fread(file.path(ENROLL_DIR, cohort_csv),
                   colClasses = list(character = "apcd_unique_id"))

# Restrict cohort to Tier 2 DFU patients only
dfu_ids     <- unique(pt$apcd_unique_id)
cohort_dfu  <- cohort_dt[apcd_unique_id %in% dfu_ids]

message("DFU (Tier 2) patients total:  ", length(dfu_ids))
message("DFU patients in ",  COHORT,
        " cohort:                       ", nrow(cohort_dfu))

# ---- Build DFU-only enrollment-weighted denominator -------------------------
# ZCTA lookup per DFU patient
zcta_lookup <- unique(pt[, .(
    apcd_unique_id,
    zcta,
    amp_year  = as.integer(format(first_amp_date, "%Y")),
    dfu_year  = as.integer(format(first_dfu_date, "%Y"))
)])
zcta_lookup[, censor_year := fifelse(
    !is.na(amp_year) & (is.na(dfu_year) | amp_year >= dfu_year),
    amp_year, NA_integer_
)]

coh <- merge(cohort_dfu[, c("apcd_unique_id", BIN_COLS), with = FALSE],
             zcta_lookup, by = "apcd_unique_id", all.x = FALSE)

long <- melt(coh, id.vars = c("apcd_unique_id", "zcta", "censor_year"),
             measure.vars = BIN_COLS,
             variable.name = "bin_col", value.name = "months")
long[, months := as.integer(months)]
long[is.na(months), months := 0L]
long[, bin_id := sub("^m_h([12])_(\\d{4})$", "\\2-H\\1", as.character(bin_col))]
long[, bin_year := as.integer(sub("^(\\d{4}).*$", "\\1", bin_id))]

# Censor post-amputation bins (patient no longer at risk for first amp)
long[!is.na(censor_year) & bin_year > censor_year, months := 0L]

if (COHORT == "continuous") {
    long[, contrib := fifelse(months >= 1L, 1.0, 0.0)]
} else {
    long[, contrib := fifelse(months >= BIN_MONTH_FLOOR, months / 6.0, 0.0)]
}

den <- long[, .(dfu_person_halfyears = sum(contrib)), by = .(zcta, bin_id)]

# ---- Build amputation numerator (first amp in bin, among Tier 2) ------------
pt_amp <- pt[!is.na(first_amp_date) &
             first_amp_date >= as.Date("2017-01-01") &
             first_amp_date <= as.Date("2022-12-31") &
             apcd_unique_id %in% cohort_dfu$apcd_unique_id]

pt_amp[, bin_year := as.integer(format(first_amp_date, "%Y"))]
pt_amp[, half     := ifelse(as.integer(format(first_amp_date, "%m")) <= 6, 1L, 2L)]
pt_amp[, bin_id   := sprintf("%d-H%d", bin_year, half)]

num <- pt_amp[, .(amp_num = .N), by = .(zcta, bin_id)]

# ---- Combine and pool -------------------------------------------------------
grid <- CJ(zcta = unique(c(den$zcta, num$zcta)), bin_id = HALF_BIN_IDS)
panel <- merge(grid, den, by = c("zcta", "bin_id"), all.x = TRUE)
panel <- merge(panel, num, by = c("zcta", "bin_id"), all.x = TRUE)
panel[is.na(dfu_person_halfyears), dfu_person_halfyears := 0]
panel[is.na(amp_num),              amp_num              := 0L]

pooled <- panel[, .(dfu_person_halfyears = sum(dfu_person_halfyears),
                    amp_person_halfyears = sum(amp_num)),
                by = zcta]
pooled[, rate_per_1000 := 1000 * amp_person_halfyears /
                          pmax(dfu_person_halfyears, 1L)]
pooled[, suppressed := dfu_person_halfyears < DEN_THRESH]
pooled_active <- pooled[!(suppressed)]

message("ZCTAs modeled (amp among DFU): ", nrow(pooled_active))
message("Raw rate range: ",
        round(min(pooled_active$rate_per_1000), 1), " - ",
        round(max(pooled_active$rate_per_1000), 1),
        " amp/1,000 DFU person-halfyears")

# ---- Spatial layer, EB, Gi*, LISA -------------------------------------------
ar_state <- states(progress_bar = FALSE)
ar_state <- ar_state[ar_state$STUSPS == "AR", ]
ar_state <- st_transform(ar_state, 5070)

ar_zctas <- zctas(year = 2020, progress_bar = FALSE)
ar_zctas$zcta <- ar_zctas$ZCTA5CE20
ar_zctas <- st_transform(ar_zctas, 5070)
ar_cent  <- st_centroid(ar_zctas)
inside   <- st_intersects(ar_cent, ar_state, sparse = FALSE)[, 1]
ar_zctas <- ar_zctas[inside, ]

zcta_sf <- merge(ar_zctas, pooled_active, by = "zcta", all.x = FALSE)

coords <- st_centroid(zcta_sf) |> st_coordinates()
nb     <- knn2nb(knearneigh(coords, k = KNN_K), sym = FALSE)
lw     <- nb2listw(nb, style = "W", zero.policy = TRUE)

eb_local <- EBlocal(zcta_sf$amp_person_halfyears,
                    zcta_sf$dfu_person_halfyears, nb)
zcta_sf$rate_raw      <- zcta_sf$rate_per_1000
zcta_sf$rate_per_1000 <- eb_local$est * 1000

set.seed(20260421)
moran_global <- moran.mc(zcta_sf$rate_per_1000, lw,
                         nsim = NPERM, zero.policy = TRUE)
print(moran_global)
saveRDS(moran_global, file.path(OUT_DIR,
        sprintf("global_morans_I_amp_among_dfu_%s.rds", COHORT)))

set.seed(20260421)
gi_perm <- localG_perm(zcta_sf$rate_per_1000, lw,
                       nsim = NPERM, zero.policy = TRUE)
zcta_sf$gi_z     <- as.numeric(gi_perm)
zcta_sf$gi_p     <- attr(gi_perm, "internals")[, "Pr(z != E(Gi))"]
zcta_sf$gi_p_fdr <- p.adjust(zcta_sf$gi_p, method = "BH")
zcta_sf$gi_bin <- with(as.data.frame(zcta_sf), fcase(
    gi_z >=  2.58,                   "Hot 99%",
    gi_z >=  1.96 & gi_z < 2.58,     "Hot 95%",
    gi_z >=  1.65 & gi_z < 1.96,     "Hot 90%",
    gi_z <= -2.58,                   "Cold 99%",
    gi_z <= -1.96 & gi_z > -2.58,    "Cold 95%",
    gi_z <= -1.65 & gi_z > -1.96,    "Cold 90%",
    default = "Not significant"
))
zcta_sf$gi_bin <- factor(zcta_sf$gi_bin, levels = c(
    "Cold 99%", "Cold 95%", "Cold 90%", "Not significant",
    "Hot 90%", "Hot 95%", "Hot 99%"))
zcta_sf$gi_fdr_robust <- zcta_sf$gi_p_fdr < FDR_ALPHA

set.seed(20260421)
lisa <- localmoran_perm(zcta_sf$rate_per_1000, lw,
                        nsim = NPERM, zero.policy = TRUE)
zcta_sf$lisa_I     <- lisa[, "Ii"]
zcta_sf$lisa_z     <- lisa[, "Z.Ii"]
zcta_sf$lisa_p     <- lisa[, "Pr(z != E(Ii))"]
zcta_sf$lisa_p_fdr <- p.adjust(zcta_sf$lisa_p, method = "BH")

mean_rate <- mean(zcta_sf$rate_per_1000, na.rm = TRUE)
lagged    <- lag.listw(lw, zcta_sf$rate_per_1000, zero.policy = TRUE)
zcta_sf$lisa_quad <- with(as.data.frame(zcta_sf), fcase(
    lisa_p >= 0.05, "NS",
    rate_per_1000 >= mean_rate & lagged >= mean_rate, "High-High",
    rate_per_1000 <  mean_rate & lagged <  mean_rate, "Low-Low",
    rate_per_1000 >= mean_rate & lagged <  mean_rate, "High-Low",
    rate_per_1000 <  mean_rate & lagged >= mean_rate, "Low-High"
))
zcta_sf$lisa_fdr_robust <- zcta_sf$lisa_p_fdr < FDR_ALPHA

saveRDS(zcta_sf, file.path(OUT_DIR,
        sprintf("zcta_sf_lisa_pooled_amp_among_dfu_%s.rds", COHORT)))

# ---- Maps -------------------------------------------------------------------
theme_map <- theme_void(base_size = 12) +
    theme(plot.title    = element_text(face = "bold", size = 14),
          plot.subtitle = element_text(color = "grey30"),
          plot.caption  = element_text(color = "grey40", size = 9),
          legend.position = "right")

p_rate <- ggplot() +
    geom_sf(data = ar_zctas, fill = "grey93", color = "grey85",
            linewidth = 0.08) +
    geom_sf(data = zcta_sf, aes(fill = rate_per_1000),
            color = "grey40", linewidth = 0.15) +
    scale_fill_viridis(option = "C",
                       name = "Amputations per\n1,000 DFU\nperson-halfyears\n(EB-smoothed)") +
    geom_sf(data = ar_state, fill = NA, color = "black", linewidth = 0.6) +
    labs(title    = "DFU \u2192 Amputation Progression \u2014 AR ZCTAs",
         subtitle = sprintf("Cohort: %s | Pooled 2017\u20132022", COHORT),
         caption  = sprintf("Numerator: first amputation among Tier 2 DFU | Denominator: DFU person-halfyears | N amputations = %d",
                            sum(pt_amp$apcd_unique_id %in% cohort_dfu$apcd_unique_id))) +
    theme_map
ggsave(file.path(OUT_DIR, sprintf("map_rate_amp_among_dfu_%s.png", COHORT)),
       p_rate, width = 9, height = 7, dpi = 300)

gi_palette <- c(
    "Cold 99%" = "#053061", "Cold 95%" = "#2166ac", "Cold 90%" = "#4393c3",
    "Not significant" = "grey88",
    "Hot 90%"  = "#f4a582", "Hot 95%"  = "#d6604d", "Hot 99%"  = "#b2182b"
)
p_gi <- ggplot() +
    geom_sf(data = ar_zctas, fill = "grey93", color = "grey85",
            linewidth = 0.08) +
    geom_sf(data = zcta_sf, aes(fill = gi_bin),
            color = "grey40", linewidth = 0.15) +
    scale_fill_manual(values = gi_palette, name = "Gi* Cluster",
                      drop = FALSE) +
    geom_sf(data = ar_state, fill = NA, color = "black", linewidth = 0.6) +
    labs(title    = "Gi* \u2014 DFU \u2192 Amputation Progression",
         subtitle = sprintf("Cohort: %s | Single-test z-bands", COHORT),
         caption  = sprintf("Local EB | KNN k=8 | 999 permutations | Global Moran\u2019s I = %.3f, p = %.3f",
                            moran_global$statistic, moran_global$p.value)) +
    theme_map
ggsave(file.path(OUT_DIR, sprintf("map_gi_amp_among_dfu_%s.png", COHORT)),
       p_gi, width = 9, height = 7, dpi = 300)

lisa_palette <- c("High-High" = "#b2182b", "Low-Low" = "#2166ac",
                  "High-Low" = "#f4a582", "Low-High" = "#92c5de",
                  "NS" = "grey88")
p_lisa <- ggplot() +
    geom_sf(data = ar_zctas, fill = "grey93", color = "grey85",
            linewidth = 0.08) +
    geom_sf(data = zcta_sf, aes(fill = lisa_quad),
            color = "grey40", linewidth = 0.15) +
    scale_fill_manual(values = lisa_palette, name = "LISA Quadrant",
                      drop = FALSE) +
    geom_sf(data = ar_state, fill = NA, color = "black", linewidth = 0.6) +
    labs(title    = "LISA \u2014 DFU \u2192 Amputation Progression",
         subtitle = sprintf("Cohort: %s | Single-test p < 0.05", COHORT),
         caption  = "Local EB | KNN k=8 | 999 permutations") +
    theme_map
ggsave(file.path(OUT_DIR, sprintf("map_lisa_amp_among_dfu_%s.png", COHORT)),
       p_lisa, width = 9, height = 7, dpi = 300)

message("\n08_amp_among_dfu_spatial.R complete.")
message(sprintf("ZCTAs modeled:    %d", nrow(zcta_sf)))
message(sprintf("Global Moran's I: %.3f (p = %.3f)",
                moran_global$statistic, moran_global$p.value))
print(table(zcta_sf$gi_bin))
print(table(zcta_sf$lisa_quad))
