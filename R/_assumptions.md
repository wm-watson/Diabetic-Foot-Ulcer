...
> includes **both Medicare FFS and Medicare Advantage**:
>
> - The seven `MCR_*_CLM` claim tables (INP, OUT, HHA, HSP, SNF, DME,
>   PRTB) are explicitly described as **fee-for-service** in the data
>   dictionary (e.g., `MCR_INP_CLM` notes: "Inpatient data contains
>   final action **fee-for-service** claims data submitted by inpatient
>   providers...").
> - `MCR_BEN_SUM` is the CMS Master Beneficiary Summary File covering
>   **all Medicare beneficiaries** (FFS and MA together). The
>   `BENE_HMO_CVRAGE_TOT_MONS` field distinguishes FFS (HMO months = 0)
>   from MA (HMO months > 0).
> - MA encounter claims are captured separately via the
>   `CLAIM_SVC_DT_YYYY` tables (where AR-licensed MA plans submit
>   alongside Commercial and Medicaid carriers).
> - MA enrollment is captured via `AR_APCD_24B_MEST` with
>   `payer_type = MCR_ADV`.
>
> **Implication:** the rev 4 §4.4 statement that "CMS-reported Medicare
> FFS *is* fully captured" is correct. The FFS-missing concern raised
> briefly on 2026-04-28 (rev 5, since reverted) does not apply. The
> Memphis-border cold spot is the rev 4 mechanism (TN-licensed MA
> plans not reporting to AR APCD) plus secondary contributions from
> Memphis VA and self-pay/charity care.
